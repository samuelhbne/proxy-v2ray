...
docker run --name $CTNNAME -p $SOCKSPORT:1080 -p $DNSPORT:53/udp -p $HTTPPORT:8123 -d $IMGNAME:$TARGET -h ${VHOST} -p ${V2RAYPORT} -u ${V2RAYUUID} -l ${V2RAYLEVEL} -a ${V2RAYAID} -s ${V2RAYSECURITY} >/dev/null
echo "Done."
echo
